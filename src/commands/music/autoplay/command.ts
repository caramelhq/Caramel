...
@ApplyOptions<Command.Options>({
    name: 'autoplay',
    description: musicEn.command.autoplay.description,
...
export class AutoplayCommand extends Command {
    public override registerApplicationCommands(registry: Command.Registry) {
        registry.registerChatInputCommand((builder) =>
            builder
                ...
                .setDescriptionLocalizations({ 'es-ES': musicEs.command.autoplay.description })
        );
    }
    ...
    public override async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
        ...
        if (!musicPlayer.autoplay) {
            musicPlayer.clearAutoplayTracks();
        }

        await musicPlayer.saveState().catch(() => null);
        await musicPlayer.refresh().catch(() => null);

        const successMsg = await resolveKey(interaction, musicPlayer.autoplay ? 'music:controls.autoplayEnabled' : 'music:controls.autoplayDisabled');
