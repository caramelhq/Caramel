...
@ApplyOptions<Command.Options>({
    name: 'tempban',
    description: 'Ban a member for a specific duration',
...
export class TempBanCommand extends Command {
    ...
    public async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
        ...
        await sendModDM({ userId: target.id, moderatorId: interaction.user.id, action: 'tempban', guild: interaction.guild!, reason, duration: duration.formatted });
        ...
        await prisma.activeTempBan.upsert({
            where:  { tempban_guild_user_unique: { guildId: interaction.guildId!, userId: target.id } },
            create: { 
                guildId: interaction.guildId!, 
                userId: target.id, 
                moderatorId: interaction.user.id, 
        ...
        const successMsg = await resolveKey(interaction, 'modcommands:sanctions.confirmations.tempban', { 
    ...
    public async messageRun(message: Message, args: Args) {
        ...
        await sendModDM({ userId: target.id, moderatorId: message.author.id, action: 'tempban', guild: message.guild!, reason, duration: duration.formatted });
        ...
        await prisma.activeTempBan.upsert({
            where:  { tempban_guild_user_unique: { guildId: message.guildId!, userId: target.id } },
            create: { 
                guildId: message.guildId!, 
                userId: target.id, 
                moderatorId: message.author.id, 
        ...
        const successMsg = await resolveKey(message, 'modcommands:sanctions.confirmations.tempban', { 
            emoji: Emojis.ban_emoji, 
            user: target.toString(), 
            userId: target.id 
        });
