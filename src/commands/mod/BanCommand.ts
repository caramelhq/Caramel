...
export class BanCommand extends Command {
    ...
    public async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
        ...
        try {
            ...
            return interaction.editReply({ ...getMessageLayout(await resolveKey(interaction, 'modcommands:mod.ban.success', { emoji: Emojis.enabled_setting_emoji, user: target.user.tag })) });
        } catch (error) {
    ...
    public async messageRun(message: Message, args: Args) {
        ...
        try {
            ...
            return message.reply({ ...getMessageLayout(await resolveKey(message, 'modcommands:mod.ban.success', { emoji: Emojis.enabled_setting_emoji, user: target.user.tag })) });
        } catch (error) {
            this.container.logger.error(`[MOD BAN]`, error);
